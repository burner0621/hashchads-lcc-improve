            ...
            if (toolTip) {
                ...
                chartCreated.subscribeCrosshairMove(function (param) {
                    if (
                        ...
                        let dateObject = new Date (param.time)
                        let dateStr = useWeekly
                            ? dayjs(dateObject.getFullYear() + '-' + (dateObject.getMonth() + 1) + '-' + dateObject.getDate())
                                .startOf('week')
                                .format('MMMM D, YYYY') +
                            '-' +
                            dayjs(dateObject.getFullYear() + '-' + (dateObject.getMonth() + 1) + '-' + dateObject.getDate())
                                .endOf('week')
                                .format('MMMM D, YYYY')
                            : dayjs(dateObject.getFullYear() + '-' + (dateObject.getMonth() + 1) + '-' + dateObject.getDate()).format('MMMM D, YYYY')
                        var price = param.seriesData.get(series)

                        toolTip.innerHTML =
                            `<div style="font-size: 16px; margin: 4px 0px; color: ${textColor};">${title}</div>` +
                            `<div style="font-size: 22px; margin: 4px 0px; color: ${textColor}">` +
                            formattedNum(price.value, true) +
                            '</div>' +
                            '<div>' +
                            dateStr +
                            '</div>'
