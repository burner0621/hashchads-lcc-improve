...
export function formattedPercent(percent, useBrackets = false) {
  ...
  if (percent < 0.0001 && percent > 0) {
    return (
      <Text fontWeight={500} color="#20eb7a">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (percent < 0 && percent > -0.0001) {
    return (
      <Text fontWeight={500} color="#ff422b">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (fixedPercent > 0) {
    if (fixedPercent > 100) {
      return <Text fontWeight={500} color="#20eb7a">{`+${percent?.toFixed(0).toLocaleString()}%`}</Text>
    } else {
      return <Text fontWeight={500} color="#20eb7a">{`+${fixedPercent}%`}</Text>
    }
  } else {
    return <Text fontWeight={500} color="#ff422b">{`${fixedPercent}%`}</Text>
  }
}
